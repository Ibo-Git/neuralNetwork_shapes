...
def evaluate(input_tensor, target_tensor, encoder, decoder, device, vocab, criterion, max_length):
    ...
    acc = get_accuracy(torch.stack(output_sequence), target_tensor, vocab)
    decoded_output_seq, decoded_expected_seq = decode_char(output_sequence, target_tensor, vocab)

    return loss.item() / target_length, acc, decoded_output_seq, decoded_expected_seq
...
def decode_char(output_sequence, target_sequence, vocab):
    ...
    # decode until padding_index is reached
    decoded_vec_output = []
...
def get_accuracy(output, target, vocab):
    # sequence
    index_pad = (target == vocab['<PAD>']).nonzero(as_tuple=True)[0]
    if len(index_pad) == 0:
        index_pad = len(target)
    else:
        index_pad = index_pad[0]
    
    acc = torch.sum(output[0:index_pad] == target[0:index_pad]) / len(target[0:index_pad])
